...
print("[*]                        - Python3 conversion Fovos 2021")
if len(sys.argv) != 4:
		print("Usage:   python3 dc_userenum_ldap.py DomainControlerIP DNSDomainName Userlist")
		print("Example: python3 dc_userenum_ldap.py 192.168.1.10 Contoso.com userlist.txt")
		sys.exit()
...
filterDomain = {'dnsdomattr': b'DnsDomain', 'dnsdomval': sys.argv[2].encode()}
filterVersion = {'ntverattr': b'NtVer', 'ntverval': '\x03\x00\x00\x00'.encode()}
filterUser = {'userattr': b'User', 'userval': b''}
filterAAC = {'aacattr': b'AAC', 'aacval': '\x10\x00\x00\x00'.encode()}
...
packet = {'baseObject': b'', 'scope': 'baseObject', 'derefAliases': 'neverDerefAliases', 'sizeLimit': 0, 'timeLimit': 0,'typesOnly': False, 'returntype': returntype, 'filters': filters}
...
for user in usernames:
	user = user.rstrip()
	message['protocolOp']['filters']['filterUser']['userval'] = user.encode()
