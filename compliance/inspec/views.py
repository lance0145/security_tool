...
def list_vuln(request):
    ...
    inspec_all_vuln = inspec_scan_results_db.objects.filter(username=username, scan_id=scan_id).values(
        'controls_id',
        'controls_title',
    ...
    inspec_all_audit = inspec_scan_results_db.objects.filter(username=username, scan_id=scan_id)
...
def inspec_vuln_data(request):
    ...
    if request.method == "POST":
        ...
        inspec_scan_results_db.objects.filter(username=username, vuln_id=vuln_id,
                                              scan_id=scan_id).update(false_positive=false_positive,
                                                                      vuln_status=status)

        if false_positive == 'Yes':
            vuln_info = inspec_scan_results_db.objects.filter(username=username, scan_id=scan_id, vuln_id=vuln_id)
            for vi in vuln_info:
                Name = vi.Name
    ...
    inspec_vuln_data = inspec_scan_results_db.objects.filter(username=username, scan_id=scan_id,
                                                             vuln_id=vuln_id,
                                                             vuln_status='Open',
                                                             false_positive='No'
                                                             )

    vuln_data_closed = inspec_scan_results_db.objects.filter(username=username, scan_id=scan_id,
                                                             vuln_id=vuln_id,
                                                             vuln_status='Closed',
                                                             false_positive='No')
    false_data = inspec_scan_results_db.objects.filter(username=username, scan_id=scan_id,
                                                       vuln_id=vuln_id,
                                                       false_positive='Yes')
...
def inspec_details(request):
    ...
    inspec_vuln_details = inspec_scan_results_db.objects.filter(username=username,
                                                                scan_id=scan_id,
                                                                vuln_id=vuln_id
                                                                )
...
def del_inspec(request):
    ...
    if request.method == 'POST':
        ...
        for i in range(0, split_length):
            ...
            item.delete()
            item_results = inspec_scan_results_db.objects.filter(username=username, scan_id=scan_id)
            item_results.delete()
...
def inspec_del_vuln(request):
    ...
    if request.method == 'POST':
        ...
        for i in range(0, split_length):
            ...
            delete_vuln = inspec_scan_results_db.objects.filter(username=username, vuln_id=vuln_id)
            delete_vuln.delete()
...
def export(request):
    ...
    if request.method == 'POST':
        ...
        queryset = inspec_scan_results_db.objects.filter(username=username, scan_id=scan_id)
